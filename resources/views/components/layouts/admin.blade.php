...
<body class="docmost-body antialiased" x-data="{ sidebarCollapsed: false, mobileOpen: false, profileOpen: false }">
    <div class="docmost-shell">
        <aside id="sidebar"
            class="docmost-sidebar transition-all duration-300"
            :class="[
                mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
                sidebarCollapsed ? 'lg:w-20' : 'lg:w-[280px]'
            ]">
            <div class="docmost-brand">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2" :class="sidebarCollapsed ? 'justify-center w-full' : ''">
                    <span class="docmost-dot"></span>
                    <span class="docmost-logo" x-show="!sidebarCollapsed" x-cloak>Doccat</span>
                </a>
            ...
            <div class="px-3 pb-3" x-show="!sidebarCollapsed" x-cloak>
                <button type="button" class="docmost-workspace-btn">
                    <span class="inline-flex h-5 w-5 rounded-full bg-rose-500 text-white text-[10px] font-bold items-center justify-center">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                    <span>{{ auth()->user()->name }}</span>
                    <svg class="h-4 w-4 text-slate-400 ml-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>
            </div>

            <nav class="flex-1 overflow-y-auto px-2 pb-4 space-y-2">
                @forelse ($menus as $menu)
                    ...
                    @if ($hasChildren)
                        ...
                        @endphp
                        <div class="docmost-group" x-data="{ open: {{ $groupActive ? 'true' : 'false' }} }" :title="sidebarCollapsed ? '{{ $menu->name }}' : ''">
                            <button type="button" class="docmost-group-toggle" x-show="!sidebarCollapsed" x-cloak @click="open = !open">
                                <span class="docmost-group-title !pb-0 !px-0">{{ $menu->name }}</span>
        ...
        <div class="docmost-main">
            <header class="docmost-topbar">
                <div class="flex items-center gap-2 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <h1 class="docmost-title truncate">{{ $pageTitle ?? 'Dashboard' }}</h1>
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    @foreach ($docToolbarActions as $action)
                        <button class="docmost-action-btn {{ $action['style'] === 'active' ? 'docmost-action-btn-active' : '' }}">{{ $action['label'] }}</button>
                    @endforeach
                    <div class="relative">
                        <button @click="profileOpen = !profileOpen" type="button" class="docmost-avatar-btn">
                        ...
                        <div x-show="profileOpen"
                            ...
                            <a href="{{ route('profile.show') }}" class="docmost-menu-item">Profil Saya</a>
                            <a href="{{ route('profile.edit') }}" class="docmost-menu-item">Edit Profil</a>
                            <a href="{{ route('profile.editPassword') }}" class="docmost-menu-item">Ubah Kata Sandi</a>
                            <form method="POST" action="{{ route('logout') }}" class="border-t border-slate-100">
                                @csrf
                                <button type="submit" class="docmost-menu-item text-rose-600">Logout</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <div class="docmost-content-wrap">
                <main class="docmost-content">
                    @if (session('success'))
                    ...
                    <div class="docmost-document">
                        {{ $slot }}
                    </div>
                </main>

                <aside class="docmost-comments hidden xl:flex">
                    <div class="docmost-comments-head">Comments</div>
                    @forelse ($docComments as $comment)
                        <div class="docmost-comment-card">
                    ...
                    @empty
                        <div class="docmost-comment-card text-xs text-slate-500">Belum ada komentar.</div>
                    @endforelse
