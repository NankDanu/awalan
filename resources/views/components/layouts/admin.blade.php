...
<body class="base_body antialiased" x-data="{ sidebarCollapsed: false, mobileOpen: false, profileOpen: false }">
    <div class="base_shell">
        <aside id="sidebar"
            class="base_sidebar transition-all duration-300"
            :class="[
                mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
                sidebarCollapsed ? 'lg:w-20' : 'lg:w-[280px]'
            ]">
            <div class="base_brand">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2" :class="sidebarCollapsed ? 'justify-center w-full' : ''">
                    <span class="base_brand_icon" aria-hidden="true">
                        <i class="ti ti-apps"></i>
                    </span>
                    <span class="base_logo" x-show="!sidebarCollapsed" x-cloak>AWALAN</span>
                </a>
            ...
            <nav class="flex-1 overflow-y-auto px-2 pt-3 pb-4 space-y-2">
                @forelse ($menus as $menu)
                    ...
                    @if ($hasChildren)
                        ...
                        @endphp
                        <div class="base_group" x-data="{ open: {{ $groupActive ? 'true' : 'false' }} }" :title="sidebarCollapsed ? '{{ $menu->name }}' : ''">
                            <button type="button" class="base_group_toggle" x-show="!sidebarCollapsed" x-cloak @click="open = !open">
                                <span class="base_group_title !pb-0 !px-0">{{ $menu->name }}</span>
        ...
        <div class="base_main">
            <header class="base_topbar">
                <div class="flex items-center gap-2 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <h1 class="base_title truncate">{{ $pageTitle ?? 'Dashboard' }}</h1>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                        @foreach ($docToolbarActions as $action)
                            <button class="base_action_btn {{ $action['style'] === 'active' ? 'base_action_btn_active' : '' }}">{{ $action['label'] }}</button>
                        @endforeach
                    @endif
                    <div class="relative">
                        <button @click="profileOpen = !profileOpen" type="button" class="base_avatar_btn">
                        ...
                        <div x-show="profileOpen"
                            ...
                            <a href="{{ route('profile.show') }}" class="base_menu_item">Profil Saya</a>
                            <a href="{{ route('profile.edit') }}" class="base_menu_item">Edit Profil</a>
                            <a href="{{ route('profile.editPassword') }}" class="base_menu_item">Ubah Kata Sandi</a>
                            <form method="POST" action="{{ route('logout') }}" class="border-t border-slate-100">
                                @csrf
                                <button type="submit" class="base_menu_item text-rose-600">Logout</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <div @class([
                ...
                @if ($showWidget)
                    <aside class="base_comments hidden xl:flex">
                        <div class="base_comments_head">Comments</div>
                        @forelse ($docComments as $comment)
                            <div class="base_comment_card">
                        ...
                        @empty
                            <div class="base_comment_card text-xs text-slate-500">Belum ada komentar.</div>
                        @endforelse
